...
def huffman( S, F ):
  ...
  for i in range(len(F)): 
    q.put((F[i], Node(F[i],S[i]))) #wypełniamy kolejkę tuplami o polu równym wartości oraz polu równym wskaźnikowi na Node utworzonego dla każdej litery

  while(q.qsize() != 1): #laczymy 2 konce w jeden dopóki nie pozostanie nam tylko 1
    first = q.get(0) 
    second = q.get(1)
    #print("merging:  ", first, "   ", second)
    parent = Node(first[0] + second[0], first[1].letter + second[1].letter, first[1], second[1]) #tworzymy nowego parenta z wczesniejszych nodów
